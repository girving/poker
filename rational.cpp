...
public:
    ...
    typedef __int128_t DI; // Double-wide integer type for detecting overflow
...
public:
    ...
    friend I rint(rational x) {
        ...
        I d = x.d();
        return (2*DI(x.n)+(x.n<0?-d+1:d-1))/(2*DI(d)); 
    }
...
template<> inline int cast(rational x) {
    return safe_cast<int>(x.n/x.d());
}

template<> inline long cast(rational x) {
    return safe_cast<long>(x.n/x.d());
}

template<> inline float cast(rational x) {
    return (float)x.n/x.d();
}

template<> inline double cast(rational x) {
    return (double)x.n/x.d();
}

template<> inline rational cast(int n) {
    return n;
}

template<> inline rational cast(long n) {
    return n;
}
...
PyObject* rational_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
    ...
    if (size==1 && PyRational_Check(x[0])) {
        Py_INCREF(x[0]);
        return x[0];
    }
    long n[2]={0,1};
    ...
    PyRational* pr = (PyRational*)type->tp_alloc(type,0);
    if (pr)
        pr->r = r;
    return (PyObject*)pr;
}
...
int rational_argmax(void* data_, npy_intp n, npy_intp* max_ind, void* arr) {
    if (!n)
        return 0;
...
PyArray_Descr rational_descr = {
    ...
    'r',                    // type
    '=',                    // byteorder
    NPY_NEEDS_PYAPI | NPY_USE_GETITEM | NPY_USE_SETITEM, // hasobject (we need the python api due to exceptions?)
    0,                      // type_num
...
#define BINARY_UFUNC(name,type,exp) \
    void rational_ufunc_##name(char** args, npy_intp* dimensions, npy_intp* steps, void* data) { \
        ...
        try { \
            for (int k = 0; k < n; k++) { \
                rational x = *(rational*)i0, y = *(rational*)i1; \
                *(type*)o = exp; \
                i0 += is0; i1 += is1; o += os; \
            } \
        } catch (const exception& e) { \
...
BINARY_UFUNC(add,rational,x+y)
BINARY_UFUNC(subtract,rational,x-y)
BINARY_UFUNC(multiply,rational,x*y)
BINARY_UFUNC(divide,rational,x/y)
BINARY_UFUNC(remainder,rational,x%y)
BINARY_UFUNC(floor_divide,rational,floor(x/y))
PyUFuncGenericFunction rational_ufunc_true_divide = rational_ufunc_divide;
BINARY_UFUNC(minimum,rational,min(x,y))
BINARY_UFUNC(maximum,rational,max(x,y))
BINARY_UFUNC(equal,bool,x=y)
BINARY_UFUNC(not_equal,bool,x!=y)
BINARY_UFUNC(less,bool,x<y)
BINARY_UFUNC(greater,bool,x>y)
BINARY_UFUNC(less_equal,bool,x<=y)
BINARY_UFUNC(greater_equal,bool,x==y)

#define UNARY_UFUNC(name,exp) \
    void rational_ufunc_##name(char** args, npy_intp* dimensions, npy_intp* steps, void* data) { \
...
UNARY_UFUNC(negative,-x)
UNARY_UFUNC(absolute,abs(x))
UNARY_UFUNC(floor,floor(x))
UNARY_UFUNC(ceil,ceil(x))
UNARY_UFUNC(trunc,cast<long>(x))
UNARY_UFUNC(square,x*x)
UNARY_UFUNC(rint,rint(x))
UNARY_UFUNC(sign,sign(x))
UNARY_UFUNC(reciprocal,inverse(x))
...
initrational() {
    ...
    if (register_cast<int, rational>(PyArray_DescrFromType(NPY_INT), npy_rational,true)<0) return;
    if (register_cast<long,rational>(PyArray_DescrFromType(NPY_LONG),npy_rational,true)<0) return;
    if (register_cast<rational,int   >(&rational_descr,NPY_INT,   false)<0) return;
    if (register_cast<rational,long  >(&rational_descr,NPY_LONG,  false)<0) return;
    if (register_cast<rational,float >(&rational_descr,NPY_FLOAT, false)<0) return;
    if (register_cast<rational,double>(&rational_descr,NPY_DOUBLE,false)<0) return;
